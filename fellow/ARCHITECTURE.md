...
│   ├── hero.php                 # ヘッダー直下の紹介の帯(トップ1ページ目のみ)
│   ├── category-nav.php         # カテゴリー導線(トップ1ページ目のみ)
│   ├── pickup.php               # ピックアップ記事(トップ1ページ目のみ)
...
| 画面 | テンプレート | 備考 |
|---|---|---|
| トップ(記事一覧) | `home.php` | 紹介の帯 + カテゴリー導線 + ピックアップ + 記事リスト + サイドバー |
...
| トップページ | 紹介の帯の表示/見出し/リード文 | チェックボックス+テキスト。空欄ならサイト名とキャッチフレーズで補う |
| トップページ | ピックアップの取得元/件数 | セレクトボックス(固定記事/最新/非表示、2〜5件) |
...
## 6.5 トップ最上部(紹介の帯 / ピックアップ)

アイキャッチを設定しない運用が前提のため、**画像に依存しない**要素で「顔」を作る。アクセントカラーの帯と、番号を振ったピックアップ枠で見せる。

- **紹介の帯**は全幅にしたいので、コンテナの内側ではなく `header.php` の `</header>` 直後に `template-parts/hero.php` を差し込む。描画するのはブログトップの1ページ目だけで、判定はテンプレート側が持つ
- **カテゴリー導線**は分類から探す読者向けの入口。ヘッダーメニューはスマホだとハンバーガーの中、サイドバーのカテゴリーは実測で1.6画面目まで下がるため、上部に独立して置く。画像バナーではなくテキストのチップにして、アイキャッチ未設定でも成立させる。スマホでは横スクロールにして縦を消費しない
- 親カテゴリーは直下に記事が無くても、子に記事があれば `hide_empty` を通り抜ける。`pad_counts` を立てないと件数が 0 と表示されるので注意
- **ピックアップ**はサイドバーのある列に入れるため `home.php` の中で呼ぶ
